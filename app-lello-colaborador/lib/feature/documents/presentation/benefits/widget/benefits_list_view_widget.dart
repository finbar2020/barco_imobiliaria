...
class BenefitsListViewWidget extends StatefulWidget {
  final List<DocumentInfo> documentsInfo;
  const BenefitsListViewWidget({
    Key? key,
    required this.documentsInfo,
  }) : super(key: key);

  @override
  State<BenefitsListViewWidget> createState() => _BenefitsListViewWidgetState();
}

class _BenefitsListViewWidgetState extends State<BenefitsListViewWidget> {
  @override
          ...
          (index) {
            return Container(
              ...
              child: InkWell(
                ...
                child: Row(
                  children: [
                    SvgPicture.asset("assets/ic_benefits.svg"),
                    SizedBox(width: Dimens.spacing),
                    Text(
                      "${getString(context, "benefits_page_list_tile_name")} ${index + 1}",
                      style: LelloTextStyles.subtitle(theme)?.copyWith(
                        color: LelloTheme.palleteOf(theme).hubText(),
