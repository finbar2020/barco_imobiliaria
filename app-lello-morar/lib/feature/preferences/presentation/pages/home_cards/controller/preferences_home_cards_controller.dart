...
class PreferencesHomeCardsController {
  ...
  Future<void> savePreferences(BuildContext context) async {
    bloc.add(PreferencesHomeCardsLoadingEvent());
    try {
      List<String> stringFavorites = favorites.map((e) => e.text()).toList();
      await preferences
  ...
  ordenedCardsList(BuildContext context) {
    List<HomeItemEnum> values = HomeItemEnum.values
        .where((element) =>
            element.checkVisible(sessionBloc) && !favorites.contains(element))
        .toList();
    favorites = favorites
    ...
    List<HomeItemEnum> ordenedByName = values
        ...
        .toList();
    ordenedByName.sort((a, b) => getString(context, a.text())
        .toUpperCase()
        .compareTo(getString(context, b.text()).toUpperCase()));
