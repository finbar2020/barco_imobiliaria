...
class PayslipEmployeesBloc
  ...
  PayslipEmployeesBloc({required this.sessionBloc, required this.listEmployee})
  ...
  Future<void> _mapNextPage(
    ...
    final lastPayslipId = data.length > 0 ? data.last.id : "";
    ...
    emit(result.fold(
        ...
        (res) => PayslipEmployeesLoadedState(
            data + res, query, condominiumId, selectedMonth, res.length == 0)));
  }
  ...
  Future<void> _mapSearch(
    ...
    emit(result.fold(
        ...
        (data) => PayslipEmployeesLoadedState(
            data, event.query, condominiumId, selectedMonth, data.length == 0)));
  ...
  void _onSessionChanged() {
    if (sessionBloc?.condominiumId != null) {
      add(PayslipEmployeesLoadEvent(condominiumId: sessionBloc!.condominiumId));
    }
  }
  ...
  void beginLoadNextPage() {
    ...
    if (!(current is PayslipEmployeesLoadingState) &&
      ...
      add(const PayslipEmployeesNextPageEvent());
    }
  }
}
