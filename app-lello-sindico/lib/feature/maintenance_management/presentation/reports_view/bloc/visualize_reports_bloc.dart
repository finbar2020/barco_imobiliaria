...
class VisualizeReportsBloc
  ...
  VisualizeReportsBloc(
    ...
    MaintenanceManagementRepository repository,
        ...
        super(VisualizeReportsInitialState()) {
  ...
  // --------------------------------------------------------------------------
  // API pública (mantida para compatibilidade com as páginas)
  // --------------------------------------------------------------------------
  ...
  Future<void> _onLoadFormularyByMonth(
    ...
    try {
      ...
      result.fold(
        (failure) {
          final s = state;
          if (s is VisualizeReportsLoadedState) {
            emit(s.copyWith(
        ...
        (response) {
          final s = state;
          if (s is VisualizeReportsLoadedState) {
            emit(s.copyWith(formularyData: response));
          } else {
            emit(VisualizeReportsLoadedState(formularyData: response));
          }
  ...
  Future<void> _onLoadReportsEfficiency(
    ...
    try {
      ...
      // Falha silenciosa: efficiency é secundária ao formulary
    }
  }
