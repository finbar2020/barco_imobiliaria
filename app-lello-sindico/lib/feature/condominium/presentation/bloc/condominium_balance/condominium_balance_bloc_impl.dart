...
class CondominiumBalanceBlocImpl extends CondominiumBalanceBloc {
  ...
  CondominiumBalanceBlocImpl(
      {required this.sessionBloc, required this.loadCondominiumBalance})
      : super(CondominiumBalanceIdleState()) {
  ...
  @override
  Stream<CondominiumBalanceState> mapEventToState(
      CondominiumBalanceEvent event) async* {
    if (event is CondominiumBalanceLoadEvent) yield* _mapLoad(event);
  }

  Stream<CondominiumBalanceState> _mapLoad(
      CondominiumBalanceLoadEvent event) async* {
    yield CondominiumBalanceLoadingState();

    var selectedCondominium = sessionBloc.state.session?.selectedCondominium;

    if (selectedCondominium == null) {
      yield CondominiumBalanceFailedState();
      return;
    }
    ...
    if (resultCache is Success<CondominiumBalance?>) {
      ...
      if (localBalance != null) {
        yield CondominiumBalanceLoadedState(
  ...
  void _onSessionChanged(SessionState state) {
    if (state is SessionLoadedState) {
      add(CondominiumBalanceLoadEvent());
    }
  }
