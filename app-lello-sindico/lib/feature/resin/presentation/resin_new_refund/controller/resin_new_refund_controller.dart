...
class ResinNewRefundController {
  final ResinNewRefundBloc bloc;
  ...
  ResinRefundsStepsEnum currentStep = ResinRefundsStepsEnum.bankAccount;
  ...
  void changeStep(ResinRefundsStepsEnum step) {
    currentStep = step;
  }
  ...
  void setUpBloc(ResinRefund? refund) {
    ...
    if (resinRefund.id != refund.id) {
      ...
      changeStep(ResinRefundsStepsEnum.valueDescription);
    }
  }
  ...
  void postRefund(ResinRefund refund) {
    switch (_action) {
      case ResinActionEnum.create:
        resinCreateRefund(refund);
        break;
      case ResinActionEnum.edit:
        resinEditRefund(refund);
        break;
    }
  }

  resinGetBankAccounts() async {
    bloc.add(ResinNewRefundLoadingEvent());
    ...
    responseCache.fold(
      (error) => bloc.add(ResinNewRefundLoadingEvent()),
      (data) {
        resinBankAccounts = data;
        bloc.add(
            ResinNewRefundLoadedEvent(bankAccounts: data, loadingRemote: true));
      },
    );
    ...
    responseRemote.fold(
      (error) => bloc.add(
          ResinNewRefundErrorEvent(errorMessageKey: "resin_get_params_error")),
      (data) {
        resinBankAccounts = data;
        resinBankAccounts = data;
        bloc.add(ResinNewRefundLoadedEvent(bankAccounts: data));
      },
    );
  }

  resinDeleteBankAccount(ResinBankAccount bankAccount) async {
    bloc.add(ResinNewRefundLoadingEvent());
  ...
  resinCreateRefund(ResinRefund refund) async {
    bloc.add(ResinNewRefundLoadingEvent());
    ...
    responseRemote.fold(
      (error) => bloc.add(ResinNewRefundErrorEvent(
          errorMessageKey: "resin_review_data_refund_error")),
      (data) {
        ManagerAnalyticsLogEvents.logEvent(
            event: AnalyticsEventsManager.solicitarReembolsoFinalizado(),
            referenceValue:
                sessionBloc.state.session?.selectedCondominium?.reference ??
                    "");
        bloc.add(ResinNewRefundSuccessEvent(data));
      },
    );
    bloc.add(ResinNewRefundLoadedEvent(bankAccounts: resinBankAccounts));
  }

  resinEditRefund(ResinRefund refund) async {
    bloc.add(ResinNewRefundLoadingEvent());
    ...
    responseRemote.fold(
      (error) => bloc.add(ResinNewRefundErrorEvent(
          errorMessageKey: "resin_review_data_refund_error")),
      (data) {
        bloc.add(ResinNewRefundSuccessEvent(refund));
      },
    );
    bloc.add(ResinNewRefundLoadedEvent(bankAccounts: resinBankAccounts));
  }

  resinCheckMaxValues() async {
    bloc.add(ResinNewRefundLoadingEvent());
    ...
    final responseRemote = await checkMaxValueUsecase.call(
      GetResinCheckMaxValueParams(
        condominiumId: condominiumId,
        type: enumToString(resinRefund.type)!,
        value: resinRefund.value,
      ),
    );
