...
import 'package:lello/environment/environment.dev2.dart';
  ...
  try {
    ...
    if (!ApplicationContainer.instance().isInitialized) {
      await ApplicationContainer.instance()
          .setUp(DevelopmentProj2Environment());
    }
...
void main() async {
  ...
  try {
    if (!TTFirebaseApp.isAppInitialized('dev2_storage')) {
      await TTFirebaseApp.initializeSecondary(
        appName: 'dev2_storage',
    ...
    print('Firebase secundário (dev2_storage) já inicializado ou erro: $e');
  }
  ...
  await ApplicationContainer.instance().setUp(DevelopmentProj2Environment());
