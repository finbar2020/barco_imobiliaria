...
Future<void> _firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  ...
  debugPrint('FCM: background message: ${message.data["is_ghost"]}');
...
Future _sendGhostNotification(RemoteMessage message) async {
  try {
    ...
    debugPrint('Error sending ghost notification: $e');
  }
}
...
Future _sendPushCallback(RemoteMessage message) async {
  try {
    ...
    debugPrint('Error sending push callback: $e');
  }
}
...
void main() async {
  ...
  // Inicializa Firebase secundário para bucket de produção (we-service)
  try {
    if (!TTFirebaseApp.isAppInitialized('prod_storage')) {
      await TTFirebaseApp.initializeSecondary(
        appName: 'prod_storage',
        config: FirebaseConfig(
          projectId: 'lello-98641',
          storageBucket: 'gs://we-service',
    ...
    debugPrint(
        'Firebase secundário (prod_storage) já inicializado ou erro: $e');
  }
