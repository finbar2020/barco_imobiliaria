...
import 'package:essentials/ui/widget/dialogs/generic_error_bottom_sheet.dart';
...
void main() {
    ...
    await pumpApp(
      ...
      Builder(
        builder: (context) => TextButton(
          onPressed: () async {
            final r = await GenericErrorBottomSheet.show<String>(
              context,
  ...
  testWidgets('show abre o sheet com ícone, mensagem e botão fechar que faz pop',
    ...
    expect(find.byType(GenericErrorBottomSheet), findsOneWidget);
    expect(find.byType(ErrorStatusIcon), findsOneWidget);
    ...
    await tester.pumpAndSettle();
    expect(find.byType(GenericErrorBottomSheet), findsNothing);
  ...
  testWidgets('golden', (tester) async {
    await pumpApp(
      tester,
      const GenericErrorBottomSheet(message: 'Mensagem do sheet com duas linhas de texto'),
      locOverrides: const {'close': 'Fechar'},
    );
    await expectLater(
      findGoldenSurface(),
      matchesGoldenFile('../goldens/generic_error_bottom_sheet.png'),
    );
  });
}
