...
import 'package:essentials/ui/widget/dialogs/generic_warning_bottom_sheet.dart';
...
void main() {
    ...
    await pumpApp(
      ...
      Builder(
        builder: (context) => TextButton(
          onPressed: () async {
            final r = await GenericWarningBottomSheet.show<String>(
              context,
  ...
  testWidgets('show abre o sheet com ícone, mensagem e botão fechar que faz pop',
    ...
    expect(find.byType(GenericWarningBottomSheet), findsOneWidget);
    expect(find.byType(WarningStatusIcon), findsOneWidget);
    ...
    await tester.pumpAndSettle();
    expect(find.byType(GenericWarningBottomSheet), findsNothing);
  ...
  testWidgets('golden', (tester) async {
    await pumpApp(
      tester,
      const GenericWarningBottomSheet(message: 'Mensagem do sheet com duas linhas de texto'),
      locOverrides: const {'close': 'Fechar'},
    );
    await expectLater(
      findGoldenSurface(),
      matchesGoldenFile('../goldens/generic_warning_bottom_sheet.png'),
    );
  });
}
